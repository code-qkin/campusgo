...
const handleSubmit = async () => {
  ...
  saving.value = true
  try {
    const data = await adminApi.patch('/user/password', {
...
<template>
  <div class="h-full overflow-y-auto p-8">
    <div class="max-w-md space-y-6">
      <div class="bg-[#1c1b1b] border border-[#2d2d2d] rounded-xl p-6 space-y-5">
        <h2 class="text-sm font-bold text-on-surface flex items-center gap-2">
          <Lock class="w-4 h-4 text-brand-primary-container" /> Change Password
        </h2>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="space-y-1.5">
            ...
            <input type="password" v-model="currentPassword"
              class="w-full bg-[#131313] border border-[#2d2d2d] rounded-lg px-3.5 py-2.5 text-sm text-on-surface focus:outline-none focus:border-brand-primary-container"
          ...
          <button type="submit" :disabled="saving"
            class="w-full py-2.5 bg-brand-primary-container text-white font-bold text-sm rounded-lg hover:brightness-110 cursor-pointer border-none disabled:opacity-60 transition-all">
