...
const pageTitle = computed(() => {
  switch (route.path) {
    case '/home':       return 'Live Network Map';
    case '/carpool':    return 'Carpool Hub';
    case '/lost-found': return 'Lost & Found Hub';
    case '/history':    return 'Activity Logs';
    case '/settings':   return 'User Preferences';
    default:            return 'CampusGo Transit';
  }
});

const searchPlaceholder = computed(() => {
  switch (route.path) {
    case '/home':       return 'Search routes, buildings...';
    case '/carpool':    return 'Search rides, locations...';
    case '/lost-found': return 'Search lost items...';
    default:            return 'Search CampusGo...';
  }
});
</script>
...
<template>
  <header class="fixed top-0 right-0 max-md:w-full w-[calc(100%-280px)] h-16 bg-[#131313] border-b border-[#2d2d2d] flex justify-between items-center max-md:px-4 px-10 z-30">
    <div class="flex items-center gap-6">
      ...
      <h2 class="text-xl font-bold text-on-surface select-none hidden lg:block tracking-wide">{{ pageTitle }}</h2>
    ...
    <div class="flex items-center gap-4 relative">
      <button @click="alert('Notification panel: No new critical transit alerts.')"
        class="w-10 h-10 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-[#2a2a2a]/40 hover:text-brand-primary transition-all relative cursor-pointer border-none bg-transparent">
        <Bell class="w-5 h-5" />
      ...
      <button @click="alert('Help Center: Accessing CampusGo Frequently Asked Questions.')"
        class="w-10 h-10 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-[#2a2a2a]/40 hover:text-brand-primary transition-all cursor-pointer border-none bg-transparent">
        <HelpCircle class="w-5 h-5" />
      </button>
      <div class="w-px h-6 bg-[#2d2d2d] mx-1"></div>

      <button @click="profileOpen = !profileOpen"
        class="flex items-center gap-3 hover:opacity-85 transition-opacity cursor-pointer text-left select-none outline-none border-none bg-transparent">
        <div class="w-8 h-8 rounded-full overflow-hidden border border-[#2d2d2d]">
          <img alt="User Profile" class="w-full h-full object-cover" :src="user.avatar" />
        </div>
        <span class="hidden md:inline text-xs font-semibold text-on-surface select-none">{{ user.fullName }}</span>
      ...
      <template v-if="profileOpen">
        ...
        <div class="absolute right-0 top-12 w-52 bg-[#1e1e1e] border border-[#2d2d2d] rounded-lg p-2 shadow-2xl z-50 flex flex-col gap-1">
          ...
          <button @click="profileOpen = false; alert(`Viewing account as ${user.fullName}`);"
            class="w-full flex items-center gap-2.5 px-3 py-2 rounded text-xs text-on-surface-variant hover:text-white hover:bg-[#2a2a2a]/40 text-left font-medium cursor-pointer border-none bg-transparent">
            <UserIcon class="w-4 h-4" /><span>My Profile</span>
          </button>
          <button @click="profileOpen = false; emit('logout');"
            class="w-full flex items-center gap-2.5 px-3 py-2 rounded text-xs text-brand-error hover:bg-brand-error-container/10 text-left font-bold cursor-pointer border-none bg-transparent">
