...
<template>
  <nav
    class="max-md:hidden fixed left-0 top-0 h-full w-[280px] bg-[#1c1b1b] border-r border-[#2d2d2d] flex flex-col p-4 z-40">
    <!-- Brand Header -->
    ...
    <ul class="flex flex-col gap-1.5 flex-1 overflow-y-auto pr-1">
      <li v-for="tab in tabs" :key="tab.id">
        <button @click="emit('change-tab', tab.id)"
    ...
    <div class="mt-auto flex flex-col gap-3 pt-4 border-t border-[#2a2a2a]/50">
      ...
      <!-- SOS disabled - coming soon -->
      <div
        class="w-full bg-[#2a2a2a] text-on-surface-variant/50 font-bold text-sm py-3.5 rounded-lg border border-[#2d2d2d] flex justify-center items-center gap-2 cursor-not-allowed">
