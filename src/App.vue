...
const activeTab = ref<ActiveTab>(ActiveTab.Welcome);
...
const handleLogout = async () => {
  const confirmLogout = await customConfirm('Logout Confirmation', 'Are you sure you want to log out of the CampusGo system?');
  if (confirmLogout) {
...
<template>
  ...
  <!-- Welcome Screen -->
  <WelcomeScreen 
    v-if="activeTab === ActiveTab.Welcome" 
    @get-started="setActiveTab(ActiveTab.Login)" 
  />

  <!-- Login/Sign Up Auth screen -->
  <AuthScreen 
    v-else-if="activeTab === ActiveTab.Login || activeTab === ActiveTab.SignUp"
    :initial-mode="activeTab === ActiveTab.Login ? 'login' : 'signup'" 
  ...
  <div v-else class="min-h-screen bg-[#131313] text-on-surface flex font-sans overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <main class="flex-1 min-h-0 max-md:pb-16">
        ...
        <RewardsHub 
          v-else-if="activeTab === ActiveTab.Rewards"
          :user="user" 
          @update-user-points="updateUserPoints" 
        />
        <HistoryHub v-else-if="activeTab === ActiveTab.History" />
        <SettingsHub 
          v-else-if="activeTab === ActiveTab.Settings"
          :user="user" 
    ...
    <div class="hidden max-md:flex fixed bottom-0 left-0 right-0 h-16 bg-[#1c1b1b] border-t border-[#2d2d2d] justify-around items-center z-50 shadow-[0_-4px_24px_rgba(0,0,0,0.5)]">
      <button @click="setActiveTab(ActiveTab.Home)" class="flex flex-col items-center p-2" :class="activeTab === ActiveTab.Home ? 'text-brand-primary' : 'text-on-surface-variant'">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      </button>
      <button @click="setActiveTab(ActiveTab.Routes)" class="flex flex-col items-center p-2" :class="activeTab === ActiveTab.Routes ? 'text-brand-primary' : 'text-on-surface-variant'">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 6v6"/><path d="M15 6v6"/><path d="M2 12h19.6"/><path d="M18 18h3s.5-1.7.8-2.8c.1-.4.2-.8.2-1.2 0-.4-.1-.8-.2-1.2l-1.4-5C20.1 6.8 19.1 6 18 6H4a2 2 0 0 0-2 2v10h3"/><circle cx="7" cy="18" r="2"/><circle cx="17" cy="18" r="2"/></svg>
      </button>
      <!-- SOS trigger -->
      <button @click="triggerSOS" class="flex flex-col items-center p-3 -mt-6 bg-[#93000a] rounded-full border-4 border-[#131313] text-[#ffb4ab] shadow-lg cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 12.2V6.5a5 5 0 0 1 10 0v5.8"/><path d="M5.7 15a4.3 4.3 0 0 0 .8 8.6h11a4.3 4.3 0 0 0 .8-8.6c0-3.9-3.5-7.4-6.3-7.4S5.7 11.1 5.7 15Z"/><circle cx="12" cy="9" r="1"/></svg>
      </button>
      <button @click="setActiveTab(ActiveTab.SafeWalk)" class="flex flex-col items-center p-2" :class="activeTab === ActiveTab.SafeWalk ? 'text-brand-primary' : 'text-on-surface-variant'">
      ...
      <button @click="setActiveTab(ActiveTab.Settings)" class="flex flex-col items-center p-2" :class="activeTab === ActiveTab.Settings ? 'text-brand-primary' : 'text-on-surface-variant'">
    ...
    <div v-if="mobileMenuOpen" class="hidden max-md:block fixed inset-0 z-[60] bg-[#131313] animate-in slide-in-from-left duration-200">
      <div class="flex flex-col h-full relative">
        <div class="h-16 flex items-center justify-between px-4 border-b border-[#2d2d2d] shrink-0">
          <h1 class="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-secondary tracking-tighter">
            CampusGo
          </h1>
          <button @click="mobileMenuOpen = false" class="p-2 text-on-surface-variant hover:text-white border-none bg-transparent">
            <X class="w-6 h-6" />
          </button>
        </div>
    ...
    <div v-if="sosActive" class="fixed inset-0 z-[100] flex items-center justify-center p-8 select-none">
      <div class="absolute inset-0 bg-black/95 backdrop-blur-md"></div>
      
      <div class="relative bg-[#201f1f] border border-[#ff5f52]/40 rounded-2xl p-10 max-w-[500px] w-full text-center shadow-[0_0_60px_rgba(255,95,82,0.3)] animate-in scale-in-95 duration-200">
        
        <div class="w-20 h-20 rounded-full bg-[#93000a] text-white flex items-center justify-center mx-auto mb-6 shadow-xl border border-[#ff5f52]/30">
        ...
        <div v-if="!sosDispatched">
          ...
          <p class="text-sm text-on-surface-variant font-light mt-3 leading-relaxed max-w-sm mx-auto">
            Broadcasting status coordinates directly to <span class="text-[#ffb4ab] font-bold">University Security & Dispatchers</span> in...
          </p>
        ...
        <div v-else class="animate-in fade-in duration-300">
          ...
          <div class="bg-[#93000a]/15 border border-[#ffb4ab]/25 p-4.5 rounded-xl my-6 text-left space-y-2.5">
            <div class="flex justify-between items-center text-xxs font-bold text-[#ffb4ab] uppercase tracking-wide">
              ...
              <span class="flex items-center gap-1.5 animate-pulse"><span class="w-2 h-2 rounded-full bg-brand-primary"></span>Active Live Stream</span>
            </div>
            <div>
              ...
              <p class="text-xxs text-on-surface-variant leading-relaxed mt-1 font-light">
                Current coordinate lock matches your phone GPS beacon. Operational dispatch squad estimated response time is under <span class="text-[#ffb4ab] font-semibold">2 minutes</span>.
              </p>
            </div>
          </div>
